...
Index get_test_index_A()
{
  ...
  // Create index.
  auto index = Index{};
  index.cluster_policy = policy;
  index.node_policy = policy;
  index.sc = sc;
  index.sn = sc;
  index.L = 1;
  index.size = 3;
...
TEST(
    maintenance_tests,
    insert_given_descriptor_and_valid_minimal_index_Sc1_AbsoluteStrategy_inserts_descriptor_reclusters_and_grows_index)
{
  // Arrange
  distance::set_distance_function(distance::Metric::EUCLIDEAN_OPT_UNROLL);
  globals::g_vector_dimensions = 3;
  ...
  auto sc = 1;
  auto root = Node{Point(new float[3]{4, 4, 4}, 0)};
  auto descriptor = Node{Point(new float[3]{5, 5, 5}, 1)};

  Index* index = maintenance::build_minimal_index(root.get_leader()->descriptor, sc, policy, policy);
...
TEST(maintenance_tests,
     build_minimal_index_given_root_node_any_sc_and_any_reclusttering_policies_return_valid_index)
{
  globals::g_vector_dimensions = 3;
  ...
  auto result = *maintenance::build_minimal_index(root.get_leader()->descriptor, sc, policy, policy);

  EXPECT_EQ(result.L, 1);
  EXPECT_EQ(result.cluster_policy, policy);
  EXPECT_EQ(result.node_policy, policy);
  EXPECT_EQ(result.sc, sc);
  EXPECT_EQ(result.sn, sc);
  EXPECT_EQ(result.size, 1);
  EXPECT_EQ(*result.root.get_leader()->descriptor, *root.get_leader()->descriptor);
  EXPECT_EQ(result.root.get_leader()->id, 0);
}
...
TEST(maintenance_helpers_tests,
     collect_path_to_nearest_clusters_given_2L_index_root_returns_stack_with_3_node_ptrs)
{
  // arrange
  distance::set_distance_function(distance::Metric::EUCLIDEAN_OPT_UNROLL);
  globals::g_vector_dimensions = 3;
  ...
  ASSERT_EQ(assert_result, 2);  // This must hold for this test to be valid.
...
TEST(maintenance_helpers_tests, count_descriptors_given_parent_with_2_cluster_with_2_points_each_returns_4)
{
  ...
  // Act
  auto result = maintenance_helpers::count_descriptors(&root);
