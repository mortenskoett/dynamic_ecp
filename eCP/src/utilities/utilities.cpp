...
// Uses a set when checking for seen samples but returns a vector for
// convenience. Time complexity is O(amount), space complexity is 2*amount due
// to the convenience vector as return type.
std::vector<unsigned> get_random_unique_indexes(unsigned amount,
                                                unsigned container_size) {
  assert(amount <= container_size &&
         "Amount must be less than or equal to container_size");

  std::vector<unsigned> collected_samples;
  collected_samples.reserve(amount);

  std::unordered_set<unsigned> visited_samples;
  std::random_device random_seed;  // Will be used to obtain a seed for the
                                   // random number engine.
  std::mt19937 generator(
      random_seed());  // Standard mersenne_twister_engine seeded with rd().
  unsigned start = container_size - amount;
  ...
  for (unsigned j = start; j < container_size; ++j) {
    ...
    std::unordered_set<unsigned>::const_iterator iter = visited_samples.find(t);
...
std::vector<std::vector<float>> generate_descriptors(
    const unsigned int count, const unsigned int dimension,
